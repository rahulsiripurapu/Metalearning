...
Mini Batch update, Adam, Shuffling batches, Quadratic.
"""
...
#   for i in range(len(x1)):
  ...
  return o
def gen_q_seq(x1,x2,mini,maxi,mino,maxo):
  ...
  y = np.linspace(mini,maxi,100)
  gx,gy = np.meshgrid(x,y)
  ot = a*gx**2 + b*gy**2 + c*gx*gy + d*gx + e*gy + f
  omax = np.max(ot)
  omin = np.min(ot)
#   o = np.zeros(len(x1))
#   for i in range(len(x1)):
  o = ((a*x1**2 + b*x2**2 + c*x1*x2 + d*x1 + e*x2 + f) - omin)*(maxo-mino)/(omax-omin) + mino
  return o
...
def generate_batches(n_batches, batch_size , sequence_length):
  ...
  for i in range(n_batches):
    ...
    for j in range(batch_size):
      ...
      y = gen_q_seq(x[:,0],x[:,1],-1,1,0.2,0.8)
      y_ = np.roll(y,1)
      x = np.vstack((x.T,y_))
      batch = batch + [x.T]
      ybatch = ybatch + [y]
...
cell = [tf.contrib.rnn.LSTMCell(num_units=hidden_units),tf.contrib.rnn.LSTMCell(num_units=hidden_units2)]
cells = tf.contrib.rnn.MultiRNNCell(cell)
init_state = cells.zero_state(batch_size, dtype=tf.float32)
cell_out,cell_state = tf.nn.dynamic_rnn(cells,X,initial_state=init_state)
outputs_flat = tf.reshape(cell_out, [-1, hidden_units2])

Wout = tf.Variable(tf.truncated_normal(shape=(hidden_units2, 1), stddev=0.1))
